"""Run only the remaining fix steps: DT retrain, backtesting, benchmark, stats."""
import sys, os, time, gc, logging, json, csv
...
try:
    ...
    logger.info("[DONE] DT: %.1f min", (time.time()-t)/60)
...
# 3. Full benchmark
logger.info("[START] Full benchmark")
try:
    ...
    def eval_scripted(task_id, seed):
        ...
        for ep in range(N_EPS):
            ...
            step = 0
            while not obs.done:
                obs = env_core.step(scripted_choose(obs, step))
                step += 1
    ...
    def eval_qrdqn(task_id, seed):
        ...
        for suffix in [task_key, "easy"]:
            ...
            if p.exists(): break
        if not p.exists(): return eval_scripted(task_id, seed)
        ...
        for ep in range(N_EPS):
            ...
            while True:
                ...
                obs, r, term, trunc, info = env.step(np.array([flat//40, flat%40], dtype=np.int64))
                total_r += r
                if term or trunc: break
            scores.append(total_r)
            env.close()
    ...
    AGENTS = {"random": eval_random, "bc": eval_bc, "scripted": eval_scripted, "qrdqn_cvar": eval_qrdqn}
    ...
    with open("benchmark/results/benchmark_summary.csv", "w", newline="") as f:
        w = csv.writer(f)
        ...
        for agent_name in AGENTS:
            ...
            for task_id in TASKS:
                ...
                row.append(f"{m:.3f}+/-{s:.3f}"); avgs.append(m)
            row.append(f"{np.mean(avgs):.3f}"); w.writerow(row)
    logger.info("[DONE] Benchmark")
except Exception as e:
