...
PHOENIX = ROOT / "versions/v5_phoenix"
...
def test_dpo_judge_preference_builder_importable():
    sys.path.insert(0, str(ROOT))
    from versions.v5_phoenix.roll_integration.dpo_judge import prepare_preference_data
...
def test_roll_env_wrapper_importable():
    ...
    try:
        from versions.v5_phoenix.roll_integration.env import supplymind_roll_env  # noqa: F401
...
def test_reward_bridge_importable_without_roll():
    sys.path.insert(0, str(ROOT))
    from versions.v5_phoenix.roll_integration.reward_bridge import supplymind_judge_worker
...
def test_phoenix_app_builds():
    sys.path.insert(0, str(ROOT))
    from versions.v5_phoenix.server import phoenix_app
