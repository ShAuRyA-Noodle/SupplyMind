...
offline-mode issue. This module is the drop-in fix:
    ...
    python -m versions.v4_arcadia_live.features.lora_train --dry-run   # no GPU
    python -m versions.v4_arcadia_live.features.lora_train --train     # 2-3h on RTX 4080
...
def build_dataset() -> list[TrainingExample]:
    ...
    mf_v5 = PROJECT_ROOT / "versions/v4_arcadia_live" / "features" / "Modelfile.analyst_v5"
    if mf_v5.exists():
...
def dry_run(config: LoRAConfig) -> dict:
    ...
    return {
        ...
        "next_step": ("Run `python -m versions.v4_arcadia_live.features.lora_train --train` "
