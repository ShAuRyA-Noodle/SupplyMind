...
from versions.v4_arcadia_live.scenarios import (
...
from versions.v4_arcadia_live.realtime.hormuz_endpoint import (
    ScenarioRequest,
    run_hormuz_pipeline,
)
...
UI_HTML_PATH = Path(__file__).resolve().parents[3] / "server" / "static" / "hormuz_war_room.html"
MASTER_HTML_PATH = Path(__file__).resolve().parents[3] / "server" / "static" / "master.html"
...
if router is not None:
    ...
    def war_room_orchestrate(req: WarRoomRequest) -> dict:
        ...
        if req.enable_openrouter_panel:
            try:
                from versions.v4_arcadia_live.realtime.openrouter_war_room_panel \
        ...
        if req.enable_specialist_judges:
            try:
                from versions.v4_arcadia_live.realtime.specialist_judges import (
